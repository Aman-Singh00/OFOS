...
import com.OFOS_Pom_Class.ChineseRestaurantDishPage;
...
public class OFOS_MyOrders_014 extends Base_Test {

	@Test
	public void placedOrderFromChineseRestaurant() throws InterruptedException {
		...
		clickAction(page.getChineseRestaurant());

		for (int i = 0; i < page.getAllRestaurant().size(); i++) {
			if (page.getAllRestaurant().get(i).getText().equals("Nan Xiang Xiao Long Bao")) {
				clickAction(page.getAllRestaurant().get(i));
				break;
			}
		}

		ChineseRestaurantDishPage cp = new ChineseRestaurantDishPage(driver);

		//test.log(LogStatus.PASS, cp.getAlldish().getText());
		//test.log(LogStatus.PASS, cp.getDishQuantity().getAttribute("value"));

		clickAction(cp.getAddToCart());
